...
static void es8311_init(uint8_t vol_pct) {
    ...
    es_write(0x0E, 0x02);
    es_write(0x0F, 0x44);
    es_write(0x10, 0x1C);   // HP output gain — 0x02 was near-silent
    es_write(0x11, 0x00);
    es_write(0x12, 0x00);   // DAC input = I2S; 0x02 routed wrong path
    es_write(0x13, 0x10);   // analog bias; 0x04 was wrong
    es_write(0x14, 0x1A);
    es_write(0x15, 0x00);
    es_write(0x16, 0x00);
    es_write(0x17, 0xBF);
